...
## Features
<ul>
  <li>create an account and log in; sessions are stored in the database</li>
  <li>signup checks that the username is free, both in the form validation and with a unique rule in the database</li>
  <li>create, rename and delete folders, with folders and files inside them</li>
  <li>upload, download and delete files; the files themselves live on Supabase</li>
  <li>share a folder with a link that expires after a duration the user picks</li>
...
[express-session](https://github.com/expressjs/session) for keeping users logged in between requests <br />
[Postgres](https://www.postgresql.org) as database <br />
[Prisma ORM](https://www.prisma.io) as database management system <br />
[CSS](https://developer.mozilla.org/en-US/docs/Web/CSS) for styling <br />
...
[Nodemon](https://www.npmjs.com/package/nodemon) for automatic restarting of the app <br />
[Express validator](https://www.npmjs.com/package/express-validator) for form data submission sanitization <br />
...
[prisma-session-store](https://github.com/kleydon/prisma-session-store.git) for user session persistence in db <br />
...
<b>process.processTicksAndRejections</b> is a bug that occurs when a network fetch fails with an unhandled promise rejection', according to the issue linked in the title. In this project, it occurs randomly with file uploads and only during that action. And I cannot predict it. There is no specified error whenever it gets logged and the file still gets successfully uploaded, displayed, downloaded and deleted. 
